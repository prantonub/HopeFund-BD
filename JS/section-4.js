document.addEventListener("DOMContentLoaded", () => {
  const donateBtn      = document.getElementById("donation-4");
  const donationInput  = document.getElementById("input-donation-4");
  const accountBalance = document.getElementById("accountBalance-4");
  const mainBalance    = document.getElementById("mainBalance");
  const modal          = document.getElementById("my_modal_4");
  const historyContent = document.getElementById("historyContent-4");
  const viewHistoryBtn = document.getElementById("view-history-btn-4");
  ...
  donateBtn?.addEventListener("click", () => {
    ...
    entry.className =
      "bg-green-50 border-2 border-orange-400 rounded-xl shadow p-5 text-gray-800 my-4 hover:shadow-lg transition-shadow duration-300";
    entry.setAttribute("data-region", "emergency-food");
    entry.innerHTML = `
      <h3 class="text-lg font-semibold text-orange-400">${amount} BDT Donated</h3>
      <p class="text-base mt-2 text-gray-800">
        For <span class="font-medium text-orange-400">Emergency Food Kits for Flood Victims</span>
      </p>
  ...
  viewHistoryBtn.addEventListener("click", () => {
    ...
    const section4Entries = document.querySelectorAll("#historyContent-4 > div");

    section4Entries.forEach(e => {
